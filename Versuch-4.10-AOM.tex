...
Zur technischen Umsetzung dient ein an einen Kristall montierter Piezo-Aktor, welcher beim Anlegen einer geeigneten Hochfrequenzspannung entsprechende mechanische Schwingungen ausführt. Auf der gegenüberliegenden Seite ist ein Absorber zur Verhinderung stehender Wellen angebracht.
...
\end{axis}
\end{tikzpicture}
\captionof{figure}{2 Messreihen für $\varepsilon(\theta)$, durchgeführt von Esra Bauer und Sören Link. Auf Grund eines gewissen Interpretationsspielraumes beim Ablesen der Messwerte, begründet in der räumlichen Ausdehnung des Laserreflexes in Relation zur notwendigen Häufigkeit der Messungen, wird der Fehler für die Position des Reflexes als relativ groß angenommen. Bei der Messung von Sören Link kommt zu den original-Messdaten noch ein Offset von $0.5mm$ hinzu.}
\end{figure}
\FloatBarrier
...
Für den Winkel $\theta$ benutzen wir hier die Position des ersten Peaks, $\theta = (0,325 \pm 0,0315)$°. Die verwendete Schallfrequenz haben wir auf $\nu_s\left( U_F= 10V \right) = \left( 94,35 \pm 1,8 \right)MHz$ bestimmt. Dies gibt uns eine Schallgeschwindigkeit von $v_s = 5262m/s \pm 29227m/s$, was deutlich über dem Literaturwert von $4200m/s$ liegt. 
